class Sha256:
    # valori constante utilizate in operatile pe biti pt. SHA 256
    ks = [
    ...
    # Este folosit pt. operatii cu biti pe int - uri de 32 de biti
    M32 = 0xFFFFFFFF

    # mlen (message length), buf (message buffer), k (constant values), h (intermediate hash value), si fin (boolean
    # care determina daca a fost procesat ultimul bloc de biti).
    ...
    def digest(self):
        ...
        if not self.fin:
            ...
            digest = b''.join(x.to_bytes(4, 'big') for x in self.h[:8])
            self.fin = True
        return digest
...
def test():
    data = input("Input data to be hashed: ")
    output = Sha256(data).hexdigest()
    print("\n SHA256 hash is : " + output)
